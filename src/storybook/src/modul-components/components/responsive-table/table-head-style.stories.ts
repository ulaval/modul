...
import { COMPLEX_TABLE_HEAD_ROWS, DEFAULT_HEAD_ROWS } from './responsive-table-data';

export default {
    title: `${modulComponentsHierarchyRootSeparator}${RESPONSIVE_TABLE_NAME}/PropHeadStyle`,
...
const getTemplate: (
    ...
    headRows: MTableHeadRows
        ...
        return Vue.extend({
            ...
            template: `<div>
        <template v-for="(headStyle, index) in headStyles">
            <p
...
export const AllHeadStyle: () => VueConstructor = (): VueConstructor => {
    return getTemplate(DEFAULT_HEAD_ROWS);
};
