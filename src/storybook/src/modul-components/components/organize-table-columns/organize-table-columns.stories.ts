...
storiesOf(`${modulComponentsHierarchyRootSeparator}${ORGANIZE_TABLE_COLUMNS_NAME}`, module)
    .add('Default', () => ({
        ...
        template: `<m-organize-table-columns @reorganize="reorganize" @reset=reset :columns="columns"></m-organize-table-columns>`
    }))
    .add('Fixed columns', () => ({
        props: {
            columns: {
                default: [
                    { id: 'check', title: '', dataProp: 'check', width: '16px', fixed: true },
