...
// Script jenkins pour la publication des packages de modul (prerelease)
		...
		stage('NPM publish') {
			steps {
				withNPM(npmrcConfig: NPM_CONFIG) {
					...
					sh "yarn run lerna publish ${params.version} --preid ${params.prereleaseid}  --dist-tag ${params.disttag} --yes"
				}
			}
		}
