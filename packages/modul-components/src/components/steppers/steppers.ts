...
import {
  ...
  MSteppersItem,
  MSteppersItemState,
} from "./steppers-item/steppers-item";
import WithRender from "./steppers.html?style=./steppers.scss";

@WithRender
@Component({
  mixins: [ElementQueries, MediaQueries],
})
export class MSteppers extends BaseSteppers {
  ...
  public setLineWidth(): void {
    ...
    for (let i: number = this.$children.length - 1; i >= 0; i--) {
      if (
        (hasFindFirst === false &&
          this.$children[i].$props.state === MSteppersItemState.InProgress) ||
        (hasFindFirst === false &&
          this.$children[i].$props.state === MSteppersItemState.Visited)
      ) {
        let childWidth: number = (this.$children[i].$el as HTMLElement)
          .clientWidth;
        let childOffset: number = (this.$children[i].$el as HTMLElement)
          .offsetLeft;
        this.lineWidth =
          ((childOffset - leftSpacing + childWidth / 2) / parentWidth) * 100;
        hasFindFirst = true;
      }
    }
  ...
  private initWidthAndHeightOfSteppers(): void {
    ...
    let overflowWrapperStyleHeight: any = (this.$refs
      .overflowWrapper as HTMLElement).style.height;
  ...
  private setMinWidth(): void {
    ...
    minWidth =
      childsWidth +
      (numberOfChild - 1) * (this.as<MediaQueries>().isMqMaxXS ? 0 : 24);
  ...
  private centeringElement(element): void {
    (this.$refs.overflowWrapper as HTMLElement).scrollLeft =
      element.offsetLeft - (this.$el.clientWidth / 2 - element.clientWidth / 2);
  }
